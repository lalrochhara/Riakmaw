...
from Riakmaw.util.tg import fetch_permissions, get_text, reply_and_delete
...
def create(func: FilterFunc, name: Optional[str] = None, **kwargs: Any) -> CustomFilter:
    return type(
        name or func.__name__ or "CustomRiakmawFilter", (CustomFilter,), {"__call__": func, **kwargs}
    )()
...
def _create_filter_permission(name: str, *, include_bot: bool = True) -> Filter:
    async def func(flt: CustomFilter, client: Client, message: Message) -> bool:
        ...
        try:
            ...
            flt.Riakmaw.log.error(f"{name} is not a valid permission")
            return False

        flt.Riakmaw.loop.create_task(
            reply_and_delete(
                message, await get_text(flt.Riakmaw, message.chat.id, "err-perm", name), 5
            )
        )
        return False
...
def _staff_only(include_bot: bool = True, *, rank: Optional[str] = None) -> CustomFilter:
    async def func(flt: CustomFilter, _: Client, message: Message) -> bool:
        ...
        if rank is None:
            return target.id in flt.Riakmaw.staff

        if rank == "dev":
            return target.id in flt.Riakmaw.devs

        flt.Riakmaw.log.error(f"Invalid rank: {rank}")
        return False
...
def _owner_only(include_bot: bool = True) -> CustomFilter:
    async def func(flt: CustomFilter, _: Client, message: Message) -> bool:
        ...
        return target.id == flt.Riakmaw.owner
...
# { admin_only
async def _send_error(robot: "Riakmaw", chat_id: int, message: Message, string_key: str) -> None:
...
def _admin_only(include_bot: bool = True, send_error: bool = True) -> CustomFilter:
    async def func(flt: CustomFilter, client: Client, message: Message) -> bool:
        ...
        if not bot_perm:
            if send_error:
                await _send_error(flt.Riakmaw, message.chat.id, message, "err-im-not-admin")

            return False

        if not member_perm:
            if send_error:
                await _send_error(flt.Riakmaw, message.chat.id, message, "err-not-admin")
        ...
        if send_error:
            if not bot_admin and user_admin:
                ...
                await _send_error(flt.Riakmaw, message.chat.id, message, "err-perm")
