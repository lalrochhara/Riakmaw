...
class TelegramBot(MixinBase):
    ...
    async def init_client(self: "Riakmaw") -> None:
        ...
        file = AsyncPath("Riakmaw/Riakmaw.session")
    ...
    async def start(self: "Riakmaw") -> None:
        if self.__running:
    ...
    async def idle(self: "Riakmaw") -> None:
        if self.__running:
            raise RuntimeError("This bot instance is already running")
    ...
    async def run(self: "Riakmaw") -> None:
        if self.__running:
            raise RuntimeError("This bot instance is already running")
    ...
    def update_plugin_events(self: "Riakmaw") -> None:
    ...
    @property
    def events_activated(self: "Riakmaw") -> int:
        return len(self._plugin_event_handlers)

    def redact_message(self: "Riakmaw", text: str) -> str:
    ...
    @cached(ttl=60)
    async def get_chat(self: "Riakmaw", chat_id: int) -> Union[Chat, ChatPreview]:
        """Wrapper for `Client.get_chat` with a TTL cache."""
        return await self.client.get_chat(chat_id)

    @cached(ttl=60)
    async def get_chat_member(self: "Riakmaw", chat_id: int, user_id: int) -> ChatMember:
